...
(function (PV) {
  ...
  let st = { cat: 'character', q: '', tag: '', sort: 'name', base: '' };
  ...
  function filtered() {
    ...
    let arr = pool().filter((i) => {
      ...
      if (!q) return true;
      return [i.name, i.prompt, i.triggers, i.file, i.notes, ...(i.tags || [])].join(' ').toLowerCase().includes(q);
    });
    const sorters = {
        ...
        <div class="lib-main">
          <div class="toolbar">
            ...
            <select class="input" data-sort>
              ${[['name', 'A → Z'], ['newest', 'Newest'], ['used', 'Most used'], ['recent', 'Recently used']].map(([v, l]) => `<option value="${v}" ${v === st.sort ? 'selected' : ''}>${l}</option>`).join('')}
            </select>
          ...
          <div class="grid" data-grid>
            ${list.map((i) => card(i)).join('') || `<div class="empty">
              ${st.q || st.tag ? 'No matches.' : isLoraCat
